#include <assert.h>
#include <stdio.h>
#include <stdlib.h>

#define STARTING_CAPACITY 8

typedef struct DA {
  // need a pointer to ANYTHING!
  void** items; // pointer to starting location of pointers. void pointer is size 8 bytes? so the pointer is not stored in the struct in memory
  int length; // number of items in the array
  int capacity; // number of items the array can hold
  // TODO define our struct
} DA;


DA* DA_new (void) { // returns a pointer to a DA struct
  DA* da = malloc(sizeof(DA)); // this is a pointer ofcouurse its just a mem alloc, pointer to memory
  da->items = malloc(STARTING_CAPACITY * sizeof(void*));
  da->length = 0;
  da->capacity = STARTING_CAPACITY;
  return da;
  // TODO allocate and return a new dynamic array
}

int DA_size(DA *da) {
  return da->length; // simple
  // TODO return the number of items in the dynamic array
}

void DA_push (DA* da, void* x) {
  if (da->length == da->capacity) {
    da->capacity *= 2; // if need to resize, double capacity
    da->items = realloc(da->items, da->capacity * sizeof(void*)); // realloc is like malloc but keeps the data
  }
  da->items[da->length] = x; // add the item to the end, where x is a pointer to the item
  da->length++;
  // TODO push to the end
}

void *DA_pop(DA *da) {
  if (da->length == 0) {
    return NULL;
  }
  void* to_return = da->items[da->length-1];
  da->length--;
  return to_return;
  // TODO pop from the end
}

void DA_set(DA *da, void *x, int i) {
  if (i >= da->length) {
    return;
  }
  da->items[i] = x;
  // TODO set at a given index, if possible
}

void *DA_get(DA *da, int i) {
  if (i >= da->length) {
    return NULL;
  }
  return da->items[i];
  // TODO get from a given index, if possible
}


void DA_free(DA *da) {
  free(da->items);
  free(da);
  // TODO deallocate anything on the heap
}

int main() {
    DA* da = DA_new();

    assert(DA_size(da) == 0);

    // basic push and pop test
    int x = 5;
    float y = 12.4;
    DA_push(da, &x);
    DA_push(da, &y);
    assert(DA_size(da) == 2);

    printf("&y = %p\n", &y);
    // printf("DA_pop(da) = %p\n", DA_pop(da));
    assert(DA_pop(da) == &y);
    assert(DA_size(da) == 1);

    assert(DA_pop(da) == &x);
    assert(DA_size(da) == 0);
    assert(DA_pop(da) == NULL);

    // basic set/get test
    DA_push(da, &x);
    DA_set(da, &y, 0);
    printf("DA_get(da, 0) = %p\n", DA_get(da, 0));
    assert(DA_get(da, 0) == &y);
    DA_pop(da);
    assert(DA_size(da) == 0);

    // expansion test
    DA *da2 = DA_new(); // use another DA to show it doesn't get overriden
    DA_push(da2, &x);
    int i, n = 100 * STARTING_CAPACITY, arr[n];
    for (i = 0; i < n; i++) {
      arr[i] = i;
      DA_push(da, &arr[i]);
    }
    assert(DA_size(da) == n);
    for (i = 0; i < n; i++) {
      assert(DA_get(da, i) == &arr[i]);
    }
    for (; n; n--)
      DA_pop(da);
    assert(DA_size(da) == 0);
    assert(DA_pop(da2) == &x); // this will fail if da doesn't expand

    DA_free(da);
    DA_free(da2);
    printf("OK\n");
}
