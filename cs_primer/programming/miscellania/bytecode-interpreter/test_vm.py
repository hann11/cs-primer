...
class TestVM(unittest.TestCase):
    def test_compute(self):
        ...
        if os.getenv("STRETCH") == "true":
            ...
            print(
                "Skipping stretch goal tests. Run `STRETCH=true python -m unittest test_vm.py` to include them."
            )
    ...
    def run_vm_test(self, test):
        ...
        for case in test.cases:
            ...
            self.assertEqual(
                actual,
                case.out,
                f"Expected f({case.x}, {case.y}) to be {case.out}, not {actual}",
            )
